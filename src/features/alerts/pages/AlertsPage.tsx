            ...
            <Card className="border-border/60 bg-gradient-to-br from-card to-secondary/40 p-6">
              ...
              <div className="mt-4 space-y-4">
                <div className="flex items-center justify-between rounded-lg bg-background/60 px-4 py-3">
                  <Label htmlFor="voice-calls" className="text-sm cursor-pointer">Voice calls</Label>
                  <Switch id="voice-calls" checked={voiceCalls} onCheckedChange={setVoiceCalls} />
                </div>
                <div className="flex items-center justify-between rounded-lg bg-background/60 px-4 py-3">
                  <Label htmlFor="sms-alerts" className="text-sm cursor-pointer">SMS alerts</Label>
                  <Switch id="sms-alerts" checked={smsAlerts} onCheckedChange={setSmsAlerts} />
                </div>
                <div className="flex items-center justify-between rounded-lg bg-background/60 px-4 py-3">
                  <Label htmlFor="app-notifications" className="text-sm cursor-pointer">App notifications</Label>
                  <Switch id="app-notifications" checked={appNotifications} onCheckedChange={setAppNotifications} />
                </div>
                <div className="flex items-center justify-between rounded-lg bg-background/60 px-4 py-3">
                  <div>
                    <Label htmlFor="quiet-hours" className="text-sm cursor-pointer">Quiet hours</Label>
        ...
        <section className="bg-muted/40 py-16">
          <div className="container mx-auto px-4">
            ...
            <div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              <IconCard icon={CloudRain} title="Weather alerts" description="Storm, frost, heatwave, and rainfall warnings." tone="sky" />
              <IconCard icon={Bug} title="Pest outbreaks" description="Early warnings for disease and insect activity." tone="destructive" />
              <IconCard icon={TrendingUp} title="Market signals" description="Price spikes or drops for your crops." tone="harvest" />
              <IconCard icon={Landmark} title="Scheme deadlines" description="Stay ahead of application windows and subsidies." tone="soil" />
            </div>
          </div>
        </section>
