...
const FORUM_POSTS = [
  {
    ...
    preview: "I have noticed the capsules are starting to turn slightly red. Should I wait another week or harvest now? What's the weather forecast saying?",
    timeAgo: "3 hours ago",
    replies: 14,
    color: "bg-primary",
  },
...
const EXPERT_QA = [
  {
    ...
    answer:
      "Katte viral disease shows as mosaic-like yellow streaks on leaves. Infected plants have stunted growth and reduced capsule production. There is no cure — remove affected plants immediately and fumigate the soil. Vector control (thrips) is critical.",
  ...
  },
  {
    ...
    answer:
      "Apply FYM (10 t/ha) at bed preparation. Follow with neem cake (1 t/ha) at planting. Top-dress with vermicompost at 30 and 60 days after planting. Avoid chemical fertilizers during heavy rain as runoff causes waste and soil acidification.",
          ...
          {FORUM_POSTS.map((post) => (
            <Card key={post.id} className="border-border/60 bg-card p-5">
              <div className="flex items-start gap-4">
                <div
                  className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-sm font-bold text-white ${post.color}`}
                >
          ...
          {VIDEOS.map((video) => (
            <Card key={video.title} className="border-border/60 bg-card overflow-hidden group cursor-pointer">
              <div className="relative flex h-36 items-center justify-center bg-muted/40">
                ...
                <span
                  className={`absolute top-3 left-3 rounded-full px-2.5 py-0.5 text-xs font-medium ${topicColors[video.topic] ?? "bg-primary/10 text-primary"}`}
                >
                  {video.topic}
                </span>
              </div>
      ...
      <section className="container mx-auto px-4 pb-20">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <IconCard
            icon={Video}
            title="Video tutorials"
            description="Step-by-step guidance in your language."
            tone="sky"
          />
          <IconCard
            icon={CalendarCheck}
            title="Seasonal guides"
            description="What to do each month for key crops."
            tone="harvest"
          />
          <IconCard
            icon={Users}
            title="Discussion forums"
            description="Ask questions, share tips, and learn together."
            tone="primary"
          />
          <IconCard
            icon={Sparkles}
            title="Expert answers"
            description="Verified agricultural officers provide advice."
            tone="soil"
          />
        </div>
      </section>
