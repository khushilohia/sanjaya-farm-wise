...
import { detectDisease } from "@/api/serverFns";
            ...
            {uploadState === "idle" && (
              ...
              >
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Take a clear photo of the affected leaf, stem, or rhizome in
                  good light.
                </p>
                <Button className="mt-5 bg-primary hover:bg-primary/90">
                  Choose photo
                </Button>
              ...
              <Card className="border-border/60 bg-gradient-to-br from-card to-secondary/40 p-6">
                ...
                <ol className="mt-4 space-y-3 text-sm text-muted-foreground">
                  ...
                  <li className="flex gap-3">
                    ...
                    Our AI vision model compares it against known regional disease
                    patterns.
                  </li>
                  <li className="flex gap-3">
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary/10 text-xs font-semibold text-primary">
                    ...
                    You get a diagnosis with confidence, severity, and a treatment
                    plan in {langName(lang)}.
                  </li>
                </ol>
                <p className="mt-5 rounded-lg bg-background/60 p-3 text-xs text-muted-foreground">
                  Tip: AI diagnosis is guidance, not a lab test. For severe or
                  spreading outbreaks, consult your local Krishi Vigyan Kendra.
                </p>
              </Card>
            )}
          </div>
              ...
              {diseaseScans.map((row) => (
                <Card key={row.id} className="border-border/60 bg-card p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      {(["Detected", "Treating", "Resolved"] as const).map(
                        (st) => (
                          <button
                            key={st}
                            type="button"
