...
export const useFarmStore = create<FarmData>()(
  persist(
    (set) => ({
      ...
      advanceSetup: (step) => set({ setupStep: step, setupComplete: step >= 4 }),
      ...
      toggleTagAILog: (id) =>
        set((s) => ({
          aiLogs: s.aiLogs.map((l) => (l.id === id ? { ...l, tagged: !l.tagged } : l)),
        })),
      ...
      updateScanStatus: (id, status) =>
        set((s) => ({
          diseaseScans: s.diseaseScans.map((d) => (d.id === id ? { ...d, status } : d)),
        })),
