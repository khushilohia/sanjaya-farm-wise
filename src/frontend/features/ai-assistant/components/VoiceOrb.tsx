...
export function VoiceOrb({ state, onClick, size = 200 }: VoiceOrbProps) {
  ...
  return (
    <button
      ...
      aria-label={state === "idle" ? "Tap to speak" : state === "listening" ? "Tap to stop" : state}
    >
      ...
      <span className="relative z-10 text-white">
        ...
        {state === "listening" && <MicOff style={{ width: iconSize, height: iconSize }} />}
        {state === "thinking" && (
          <Sparkles
        ...
        {state === "speaking" && <Volume2 style={{ width: iconSize, height: iconSize }} />}
      </span>
    </button>
  );
}
